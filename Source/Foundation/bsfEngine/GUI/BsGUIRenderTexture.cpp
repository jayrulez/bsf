...
namespace bs
{
	...
	GUIRenderTexture::GUIRenderTexture(const String& styleName, const SPtr<RenderTexture>& texture, bool transparent,
		...
		:GUITexture(styleName, SpriteTextureResourceHandle(), TextureScaleMode::StretchToFit, false, dimensions), mTransparent(transparent)
	{
	...
	void GUIRenderTexture::setRenderTexture(const SPtr<RenderTexture>& texture)
	{
		...
		if (mSourceTexture != nullptr)
		{
			...
			setTexture(SpriteTextureResource::create(texture->getColorTexture(0)));

			GUIManager::instance().setInputBridge(mSourceTexture, this);
		}
		else
		{
			setTexture(SpriteTextureResource::create(TextureResourceHandle()));
		}
