...
namespace bs
{
	...
	public:
		 ...
		 */
		BS_TEXT_EXPORT TextDataBase(const U32String& text, const HFont& font, UINT32 fontSize,
			UINT32 width = 0, UINT32 height = 0, bool wordWrap = false, bool wordBreak = true);
		BS_TEXT_EXPORT virtual ~TextDataBase() = default;

		/**	Returns the number of lines that were generated. */
		BS_TEXT_EXPORT UINT32 getNumLines() const { return mNumLines; }

		/**	Returns the number of font pages references by the used characters. */
		BS_TEXT_EXPORT UINT32 getNumPages() const { return mNumPageInfos; }

		/**	Returns the height of a line in pixels. */
		BS_TEXT_EXPORT UINT32 getLineHeight() const;

		/**	Gets information describing a single line at the specified index. */
		BS_TEXT_EXPORT const TextLine& getLine(UINT32 idx) const { return mLines[idx]; }

		/**	Returns font texture for the provided page index.  */
		BS_TEXT_EXPORT const HTexture& getTextureForPage(UINT32 page) const;

		/**	Returns the number of quads used by all the characters in the provided page. */
		BS_TEXT_EXPORT UINT32 getNumQuadsForPage(UINT32 page) const { return mPageInfos[page].numQuads; }

		/**	Returns the width of the actual text in pixels. */
		BS_TEXT_EXPORT UINT32 getWidth() const;

		/**	Returns the height of the actual text in pixels. */
		BS_TEXT_EXPORT UINT32 getHeight() const;
	...
	protected:
		 ...
		 */
		BS_TEXT_EXPORT void generatePersistentData(const U32String& text, UINT8* buffer, UINT32& size,
