...
namespace bs
{
	void generateTextures()
	{
		...
		// Save all textures
		Path outputDir = sOutputFolder + RendererBuiltinResources::TEXTURE_FOLDER;
		...
		Path whitePath = outputDir + RendererBuiltinResources::TEXTURE_WHITE_FILE;
		saveTexture(whitePath, whiteTexture, "1f7d0e3f-d81b-42ee-9d31-cb6c6fc55824");

		Path blackPath = outputDir + RendererBuiltinResources::TEXTURE_BLACK_FILE;
		saveTexture(blackPath, blackTexture, "149a5c05-9570-4915-9dbd-69acf88b865b");

		Path normalPath = outputDir + RendererBuiltinResources::TEXTURE_NORMAL_FILE;
	...
	void generateMeshes()
	{
		...
		// Save all meshes
		const Path outputDir = sOutputFolder + RendererBuiltinResources::MESH_FOLDER;
		...
		Path boxPath = outputDir + RendererBuiltinResources::MESH_BOX_FILE;
		saveMesh(boxPath, boxMesh, "bc1d20ca-7fe6-489b-8b5c-dbf798badc95");

		Path spherePath = outputDir + RendererBuiltinResources::MESH_SPHERE_FILE;
		saveMesh(spherePath, sphereMesh, "040642f3-04d6-419e-9dba-f7824161c205");

		Path conePath = outputDir + RendererBuiltinResources::MESH_CONE_FILE;
		saveMesh(conePath, coneMesh, "b8cf6db5-1736-47ac-852f-82ecd88b4d46");

		Path cylinderPath = outputDir + RendererBuiltinResources::MESH_CYLINDER_FILE;
		saveMesh(cylinderPath, cylinderMesh, "e6b2b797-4e72-7e49-61ba-4e7275bd561d");

		Path quadPath = outputDir + RendererBuiltinResources::MESH_QUAD_FILE;
		saveMesh(quadPath, quadMesh, "06592bf3-f82a-472e-a034-26a98225fbe1");

		Path discPath = outputDir + RendererBuiltinResources::MESH_DISC_FILE;
	...
	SPtr<GUISkin> generateGUISkin()
	{
		...
		const Path skinFolder = sOutputFolder + GUIBuiltinResources::SKIN_FOLDER + GUIBuiltinResources::SPRITE_FOLDER;
	...
	void processAssets(bool generateGenerated, bool forceImport, time_t lastUpdateTime)
	{
		...
		const Path rawSkinFolder = sInputFolder + GUIBuiltinResources::SKIN_FOLDER;
		const Path rawAnimatedSpritesFolder = sInputFolder + GUIBuiltinResources::ANIMATED_SPRITES_FOLDER;
		const Path rawCursorFolder = sInputFolder + GUIBuiltinResources::CURSOR_FOLDER;
		const Path rawIconFolder = sInputFolder + GUIBuiltinResources::ICON_FOLDER;
		const Path rawIcon3DFolder = sInputFolder + GUIBuiltinResources::ICON3D_FOLDER;
		const Path rawShaderFolder = sInputFolder + RendererBuiltinResources::SHADER_FOLDER;
		const Path rawShaderIncludeFolder = sInputFolder + RendererBuiltinResources::SHADER_INCLUDE_FOLDER;
		const Path rawTexturesFolder = sInputFolder + RendererBuiltinResources::TEXTURE_FOLDER;
		...
		const Path skinFolder = sOutputFolder + GUIBuiltinResources::SKIN_FOLDER;
		const Path animatedSpriteFolder = sOutputFolder + GUIBuiltinResources::ANIMATED_SPRITES_FOLDER;
		const Path cursorFolder = sOutputFolder + GUIBuiltinResources::CURSOR_FOLDER;
		const Path iconFolder = sOutputFolder + GUIBuiltinResources::ICON_FOLDER;
		const Path icon3DFolder = sOutputFolder + GUIBuiltinResources::ICON3D_FOLDER;
		const Path shaderFolder = sOutputFolder + RendererBuiltinResources::SHADER_FOLDER;
		const Path shaderIncludeFolder = sOutputFolder + RendererBuiltinResources::SHADER_INCLUDE_FOLDER;
		const Path texturesFolder = sOutputFolder + RendererBuiltinResources::TEXTURE_FOLDER;
