...
namespace bs
{
	...
	public:
		...
		Mesh(const SPtr<MeshData>& initialMeshData, const MESH_DESC& desc, GpuDeviceFlags deviceMask = GDF_DEFAULT);
		...
		SPtr<ct::VertexData> getVertexData() const override;
		 ...
		 */
		AsyncOp writeData(const SPtr<MeshData>& data, bool discardEntireBuffer, bool updateBounds = true, UINT32 queueIdx = 0);
		 ...
		 */
		virtual void writeData(const MeshData& data, bool discardEntireBuffer, bool updateBounds = true, UINT32 queueIdx = 0);
		 ...
		 * @note	This is an @ref asyncMethod "asynchronous method".
		 */
		AsyncOp readData(const SPtr<MeshData>& data, UINT32 deviceIdx = 0, UINT32 queueIdx = 0);
	...
	public:
		 ...
		 */
		static HMesh createHandle(UINT32 numVertices, UINT32 numIndices, const SPtr<VertexDataDesc>& vertexDesc, int usage = MU_STATIC, DrawOperationType drawOp = DOT_TRIANGLE_LIST, IndexType indexType = IT_32BIT, GpuDeviceFlags deviceMask = GDF_DEFAULT);
		 ...
		 */
		static SPtr<Mesh> create(UINT32 numVertices, UINT32 numIndices, const SPtr<VertexDataDesc>& vertexDesc, int usage = MU_STATIC, DrawOperationType drawOp = DOT_TRIANGLE_LIST, IndexType indexType = IT_32BIT, GpuDeviceFlags deviceMask = GDF_DEFAULT);
		 ...
		 */
		static HMesh createHandle(const MESH_DESC& desc, GpuDeviceFlags deviceMask = GDF_DEFAULT);
		/**
		 * Creates a new empty mesh.
		 *
		 * @param[in]	desc			Descriptor containing the properties of the mesh to create.
		 ...
		 *								mesh data instead.
		 */
		static HMesh createHandle(const SPtr<MeshData>& initialData, const MESH_DESC& desc, GpuDeviceFlags deviceMask = GDF_DEFAULT);
		/**
		 * Creates a new mesh from an existing mesh data. Created mesh will match the vertex and index buffers described
		 ...
		 */
		static HMesh createHandle(const SPtr<MeshData>& initialData, int usage = MU_STATIC, DrawOperationType drawOp = DOT_TRIANGLE_LIST, GpuDeviceFlags deviceMask = GDF_DEFAULT);
