...
namespace bs
{
	...
	void PrefabUtility::revertToPrefab(const HSceneObject& so)
	{
		...
		PrefabResourceHandle prefabLink = static_resource_cast<PrefabResource>(gResources().loadFromUUID(prefabLinkUUID, false, ResourceLoadFlag::None));
	...
	void PrefabUtility::updateFromPrefab(const HSceneObject& so)
	{
		...
		for (auto iter = prefabInstanceRoots.rbegin(); iter != prefabInstanceRoots.rend(); ++iter)
		{
			...
			PrefabResourceHandle prefabLink = static_resource_cast<PrefabResource>(gResources().loadFromUUID(current->mPrefabLinkUUID, false, ResourceLoadFlag::None));
