...
namespace bs
{
	...
	SPtr<RenderTexture2> RenderTexture2::create(const TEXTURE_DESC& colorDesc, bool createDepth, PixelFormat depthStencilFormat)
	{
		return TextureManager2::instance().createRenderTexture(colorDesc, createDepth, depthStencilFormat);
	}

	SPtr<RenderTexture2> RenderTexture2::create(const RENDER_TEXTURE_DESC2& desc, UINT32 deviceIdx)
	{
		return TextureManager2::instance().createRenderTexture(desc, deviceIdx);
	}

	void RenderTexture2::initialize()
	{
		...
		for (UINT32 i = 0; i < BS_MAX_MULTIPLE_RENDER_TARGETS; i++)
		{
			if (mDesc.colorSurfaces[i].texture != nullptr)
			{
				SPtr<Texture2> texture = mDesc.colorSurfaces[i].texture;
		...
		if (mDesc.depthStencilSurface.texture != nullptr)
		{
			SPtr<Texture2> texture = mDesc.depthStencilSurface.texture;
	...
	const RenderTextureProperties& RenderTexture2::getProperties() const
	{
		return static_cast<const RenderTextureProperties&>(getPropertiesInternal());
	}

	void RenderTexture2::throwIfBuffersDontMatch() const
	{
		UINT32 firstSurfaceIdx = (UINT32)-1;
		for (UINT32 i = 0; i < BS_MAX_MULTIPLE_RENDER_TARGETS; i++)
		{
		...
		{
			const TextureProperties& firstTexProps = mDesc.colorSurfaces[firstSurfaceIdx].texture->getProperties();
			SPtr<ct::TextureView> firstSurfaceView = mColorSurfaces[firstSurfaceIdx];
	...
	RenderTexture2::RenderTexture2(const RENDER_TEXTURE_DESC2& desc, UINT32 deviceIdx)
		:mDesc(desc)
	{
	}
}
