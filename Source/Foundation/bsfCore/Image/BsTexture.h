...
namespace bs
{
	...
	public:
		 ...
		 */
		static HTexture createHandle(const TEXTURE_DESC& desc, GpuDeviceFlags deviceMask = GDF_DEFAULT);

		/**
		 * @copydoc bs::Texture::create(const SPtr<PixelData>&, int, bool)
		 * @param[in]	deviceMask		Mask that determines on which GPU devices should the object be created on.
		 */
		static HTexture createHandle(const SPtr<PixelData>& pixelData, int usage = TU_DEFAULT, bool hwGammaCorrection = false, GpuDeviceFlags deviceMask = GDF_DEFAULT);
