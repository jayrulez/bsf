...
namespace bs
{
	 ...
	 */
	class BS_COREOBJECT_EXPORT CoreThread : public Module<CoreThread>
	{
	 ...
	 * @see		CoreThread
	 */
	BS_COREOBJECT_EXPORT CoreThread& gCoreThread();

	/**	Throws an exception if current thread isn't the core thread. */
	BS_COREOBJECT_EXPORT void throwIfNotCoreThread();

	/** Throws an exception if current thread is the core thread. */
	BS_COREOBJECT_EXPORT void throwIfCoreThread();
