...
namespace bs { namespace ct
{
	...
	LightGridLLCreationMat::LightGridLLCreationMat()
	{
		...
		desc.elementCount = 1;
		desc.format = BF_32X1U;//desc.format = BF_UNKNOWN;
		desc.usage = GBU_LOADSTORE;
		desc.type = GBT_STANDARD;//desc.type = GBT_STRUCTURED;
		desc.elementSize = 0;//desc.elementSize = 4;
		...
		const SPtr<GpuBuffer>& lightsBuffer, const SPtr<GpuBuffer>& probesBuffer)
	{
		...
		ClearLoadStoreMat* clearMat = ClearLoadStoreMat::getVariation(
			ClearLoadStoreType::Buffer, ClearLoadStoreDataType::Int, 1//ClearLoadStoreType::StructuredBuffer, ClearLoadStoreDataType::Int, 1
		);
