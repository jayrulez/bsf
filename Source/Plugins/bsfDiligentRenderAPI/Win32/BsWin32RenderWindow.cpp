...
namespace bs
{
	...
	{
		ct::DiligentRenderAPI& rapi = static_cast<ct::DiligentRenderAPI&>(ct::RenderAPI::instance());
	...
	namespace ct
	{
		Win32RenderWindow::Win32RenderWindow(const RENDER_WINDOW_DESC& desc, UINT32 windowId, DiligentRenderAPI& renderAPI)
			: RenderWindow(desc, windowId), mProperties(desc), mSyncedProperties(desc), mWindow(nullptr), mIsChild(false)
			, mDisplayFrequency(0), mRenderAPI(renderAPI)
		{ }
...
#else
			windowDesc.module = GetModuleHandle(L"bsfDiligentRenderAPI.dll");
#endif
